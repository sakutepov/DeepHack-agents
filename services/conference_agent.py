...
class ConferenceAgent:
    ...
    def get_conferences_from_informer(self):
        ...
        data = get_datakonferencii(
            ["106,127,128,131,132,134,137,139,142,143", "15", "#ffffff", "#008efa", "#000000", "16", "14",
             "date_creation", "desc"])
        headers = {
        ...
        moid = {mo[1]: str(mo[0] + 1) for mo in enumerate(
    ...
    def get_titles_texts(self):
        self.guap_texts = ["id:" + str(titlesid) + ", " + titletext for titlesid, titletext in
                           enumerate(list(self.guap_data['Title']))]
        self.informer_texts = ["id:" + str(titlesid + len(self.guap_texts)) + ",  " + titletext['Title'] for
                               titlesid, titletext in enumerate(list(self.informer_data))]
    ...
    def get_info_by_id(self, idx, source1, source2):
        if idx < len(source1):
            ...
            info = {tag: source2[idx - len(source1)][tag] for tag in
                    ['DateBegin', 'DateEnd', 'Title', 'Place', 'SiteLink']}
        else:
            assert False, "Указанный идентификатор выходит за границы массива"
        return info
    ...
    def process_query(self, user_query):
        ...
        while (len(conf_idx) == 0) and (counter < 3):
            ...
            answer = self.llm.invoke(
                'По запросу пользователя: [' + user_query + '] выведи чисела id наиболее подходящих конференций : ' + self.cat_text).content
            conf_idx = [int(one_char.strip()) for one_char in answer.split(',') if (one_char.strip().isdigit()) and (
                    int(one_char.strip()) < (len(self.guap_data) + len(self.informer_data)))]
        if (len(conf_idx) == 0):
            return '', None
        ...
        if len(resp) != 0:
            ...
            text_request = self.llm.invoke(
                "Напиши очень краткий анонс конференции, на которой не точно, но скорее всего будет информация по " + user_query + " : [" + textrows + "]").content
